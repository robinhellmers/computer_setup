...
\begin{enumerate}
    \item Create the \code{\textapprox/.vim/colors/} directory. 
    \item Create a file \code{mycolorscheme.vim} in \code{\textapprox/.vim/color/}.
...
\end{minted}
    ...
    \item This shows the background colors with corresponding values. Use \textbf{Page Up} and \textbf{Page down} to go through it.
        ...
        \end{figure}
    \end{enumerate}
    \item You can continuously edit \code{\textapprox/.vim/colors/mycolorscheme.vim} and see the updates of the colors while still in \code{vimdiff} by using the command \code{:colo mycolorscheme}.
    \item Now we are going to set this scheme permanently for \code{vimdiff}. Create a file in your home directory \code{\textapprox/.vimrc}.
    \newpage
